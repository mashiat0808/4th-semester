...
string input;
int bufferSize;
const uint64_t hInitial[8] = {0x6a09e667f3bcc908ULL, 0xbb67ae8584caa73bULL,0x3c6ef372fe94f82bULL,
...
void appendLength(int length, uint64_t &lo, uint64_t &hi); // appends length
void moduloF(uint64_t **buffer); // register changes
uint64_t **padding(unsigned char *input); // adds padding
void sha(); // hashing
...
void sha()
{
	uint64_t **buffer; // pura message total 16*64bits	   // 512 bit word 8 ta 8 bit
...
void moduloF(uint64_t **buffer)
{
	...
	for (int i = 0; i < bufferSize; i++)
	{
		...
		for (int j = 0; j < 80; j++)
		{
			...
			uint64_t t2 = Sig0(s[0]) + Majority(s[0], s[1], s[2]);					  // sig0(a)+maj(a,b,c)
			s[7] = s[6];														  // h=g
			s[6] = s[5];														  // g=f
			s[5] = s[4];														  // f=e
			s[4] = s[3] + t1;													  // e=d+t1
